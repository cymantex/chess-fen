...
export class Position {
  ...
  public static fromPositionOrCoordinate(
    positionOrCoordinate: Coordinate2D | string,
  ...
  public isDiagonalTo(position: Position, maxSpaces = Infinity): boolean {
    const deltaY = Math.abs(position.y - this.y);
    const deltaX = Math.abs(position.x - this.x);
  ...
  public isNorthTo(position: Position, range = 0): boolean {
    if (range) {
  ...
  public isSouthTo(position: Position, range = 0): boolean {
    if (range) {
      return this.y === position.y - range;
    }
  ...
  public isWestTo(position: Position, range = 0): boolean {
    if (range) {
  ...
  public isEastTo(position: Position, range = 0): boolean {
    if (range) {
      return this.x === position.x - range;
    }
